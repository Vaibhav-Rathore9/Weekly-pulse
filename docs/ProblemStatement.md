...
We are building an autonomous AI agent that manages a weekly “pulse” to turn public App Store and Google Play reviews for Groww into a one-page insight report. The AI agent will autonomously use MCP (Model Context Protocol) tools to append the generated report to a Google Doc and send stakeholder notifications via Gmail, rather than making ad hoc API calls.
...
Google Docs MCP — AI agent uses tools to append each week’s report as a new dated section to a single running document per product (e.g. Weekly Review Pulse — Groww). The Doc is the system of record and preserves history.
Gmail MCP — AI agent uses tools to send a short stakeholder email that includes a deep link to the new section in that Doc (heading link), not a duplicate full report in email alone.
...
The AI agent operates as an MCP client equipped with tools; it does not embed Google credentials or call the Docs/Gmail REST APIs directly. Instead, it decides when to use the Google Docs and Gmail MCP tools for delivery.
